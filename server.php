...
class G
{
    ...
    static $config = array(
        ...
        //'package_eof'              => "\r\r\n",
        //'open_cpu_affinity'        => 1,
        'socket_buffer_size'    => 134217728,//1024*1024*128,
        'buffer_output_size'    => 2097152,//1024*1024*2,
...
$process1 = new swoole_process( function( $worker ) use ( $serv )
{
    ...
    $taskList = &G::$taskList ;
    swoole_timer_tick( 2000, function( $interval ) use ( $worker, $serv ,$taskList )
    {
    ...
    swoole_timer_tick( 5000, function() use ( $serv )
    {
        ...
        $serv->sendMessage( "hello task worker", 1 );
    } );
...
function my_onReceive(swoole_server $serv, $fd, $from_id, $data)
{
    ...
    elseif( $cmd == 'tasklist')
    {
        $serv->send($fd, 'Stats: '.var_export(G::$taskList, true)."\ncount=".count($serv->connections).PHP_EOL);
    }
    else
    {
        $serv->send($fd, 'Swoole: '.$data, $from_id);
    ...
    //echo
    //"Client:Data.
    //fd=$fd|from_id=$from_id|data=$data";
    //    $serv->after(
    //        800,
    //        function
    //        ()
...
$serv->on('PipeMessage', function($serv, $src_worker_id, $msg) {
    ...
    if ($serv->taskworker)
    {
        $serv->sendMessage("hello user process", $src_worker_id);
    }
});
    ...
    function run($worker)
    {
        ...
        echo date( 'Y-m-d H:i:s' ) . ' #' . posix_getpid() . 'process will exit..' . PHP_EOL;
        $worker->exit(1);
    }
